...
│   └── (NO LIVE.JSX YET)      — Live TV page needs to be created
├── components/
...
To implement the Live TV feature you'll need to:
...
2. **`src/components/layout/Header.jsx`** and **`src/components/layout/Footer.jsx`** — Add "Live TV" navigation link
3. **`src/components/layout/Header.jsx`** — Mobile nav: add Live TV link

### Optional (for actual HLS streaming):
4. **`package.json`** — Add `hls.js` dependency if you want to play M3U8 streams inside the page rather than redirecting
5. **Create a new `LivePlayer.jsx` component** using hls.js (or skip if using redirects only)

### Architecture decision:
The user's proposed implementation uses **redirects** (opens official broadcaster sites in new tabs) rather than playing streams directly. This is the simplest, most legal approach:
- T Sports → redirects to YouTube/tsports.com
- BTV → redirects to btv.gov.bd
- Somoy TV → redirects to somoynews.tv
- FIFA+ → redirects to fifa.com
- Toffee → redirects to toffee.com.bd

No `hls.js` installation needed if using redirect-only approach. The channel data is static (`FREE_CHANNELS` array), no API calls needed.

## Suggested Implementation Order
1. Create `src/pages/Live.jsx` with the channel grid, disclaimer modal, and redirect logic
2. Create `src/styles/Live.module.css` with all the styling
3. Update `src/App.jsx` to add the `/live` route (can be eagerly loaded like Home/Movies since it has no heavy dependencies)
4. Update `src/components/layout/Header.jsx` to add "Live TV" nav link
5. Update `src/components/layout/Footer.jsx` to add "Live TV" link
6. Test navigation and channel clicks
